<table id="datatables" class="table table-striped table-bordered text-capitalize" style="white-space: nowrap; font-size: .9em;">
    ...
    <tbody>
        <?php $no = 1 ?>
        <?php foreach ($sk_domisili as $item) : ?>
            <?php $data['item'] = $item; ?>
...
<div class="modal fade" id="modal_form" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <form method="post" action="<?= base_url('surat/sk_domisili_create') ?>" enctype="multipart/form-data" autocomplete="off">
                <div class="modal-body">
                    <div class="row g-3">
                        ...
                        <div class="form-group col-lg-8 col-md-12">
                            <label for="domisili" class="form-label">Domisili Penduduk</label>
                            <textarea class="form-control text-capitalize" readonly id="domisili" rows="2"></textarea>
                        </div>
...
<script>
    ...
    const domisili = document.querySelector('#domisili')

    const setForm = (title, id = null) => {
        title_form.innerHTML = `${title} Surat Keterangan Domisili`
        const kependudukan = <?= json_encode($kependudukan) ?>;
        const data = [...<?= json_encode($sk_domisili) ?>].filter(e => e.id === id)[0];
        ...
        if (title === 'detail') {
            const list = ['no_surat', 'keperluan', 'status_print', 'foto_ktp', 'pas_foto', 'foto_kartu_pajak', 'foto_kartu_vaksin'];
