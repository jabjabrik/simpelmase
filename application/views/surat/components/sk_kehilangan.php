...
<script>
    ...
    const setForm = (title, id = null) => {
        title_form.innerHTML = `${title} Surat Keterangan Kehilangan`
        ...
        if (title === 'detail') {
            const data = [...<?= json_encode($sk_kehilangan) ?>].filter(e => e.id === id)[0];
            const list = ['no_surat', 'kehilangan', 'hari', 'lokasi', 'tanggal', 'status_print', 'foto_ktp', 'pas_foto', 'foto_kartu_pajak', 'foto_kartu_vaksin'];
